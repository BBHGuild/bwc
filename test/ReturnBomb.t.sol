...
contract Bomber {
    ...
    function calculateMemorySizeWord(uint256 memory_cost) public pure returns (uint256 memory_size_word) {
        ...
        memory_size_word = Math.sqrt(discriminant) / (2 * a) - b;
    }
...
contract Victim {
    ...
    // ❌ unsafe -- the bomb is an Error(string), so this definitely tries to load/parse
    // the revert data
    function tryCatchErrorString() external {
        ...
        // unreachable
            ...
            reason; // should not be triggered, not returning a string
            emit CaughtBomb();
        }
    }
    ...
    // ❌ unsafe (some future compiler version could avoid copying the revert data to
    // memory if it's unused)
    ...
    function lowLevelWithBytesUnused() external {
        ...
        ) = address(bomber).call(abi.encodeWithSignature("bomb()"));
        if (!success) {
            emit CaughtBomb();
        }
    }
    ...
    // see https://github.com/nomad-xyz/ExcessivelySafeCall for a wrapper library that
    // does this
