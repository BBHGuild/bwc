...
/// @notice library calculates the deployment address given a deployer and nonce
...
library LibRLP {
    ...
    function computeAddress(address deployer, uint256 nonce) internal pure returns (address) {
        // The theoretical allowed limit, based on EIP-2681, for an account nonce is
        // 2**64-2: https://eips.ethereum.org/EIPS/eip-2681.
        // However, we assume nobody can have a nonce large enough to require more than 4
        // bytes.
        // Thus, no specific check is built-in to save deployment costs.

        // The integer zero is treated as an empty byte string, and as a result it only
        // has a length prefix, 0x80, computed via 0x80 + 0.
        if (nonce == 0x00) {
            return
                address(
        ...
        // A one-byte integer in the [0x00, 0x7f] range uses its own value as a length
        // prefix, there is no additional "0x80 + length" prefix that precedes it.
        if (nonce <= 0x7f) {
            return
                address(
        ...
        // encoding scheme (the same calculation can be carried over for higher nonce
        // bytes):
        // 0xda = 0xc0 (short RLP prefix) + 0x1a (= the bytes length of: 0x94 + address +
        // 0x84 + nonce, in hex),
